...
public enum Filter {
   CUT_OFF_TERRITORY("No Route", territory -> territory.getRoute().isEmpty(), ChatFormatting.RED),
   MULTIPLE_BONUSES("Multiple Bonuses", Filter::hasMultipleBonuses, ChatFormatting.WHITE),
   ...
   PLACEHOLDER(" ", t -> false, ChatFormatting.WHITE),
   STRICT_MODE("Strict Mode", t -> false, ChatFormatting.WHITE);
   ...
   Filter(String name, Predicate<TerritoryEco> predicate, ChatFormatting textColor, CustomColor color) {
      this.name = name;
   ...
   Filter(String name, Predicate<TerritoryEco> predicate, ChatFormatting color) {
      this(name, predicate, color, CustomColor.fromChatFormatting(color).withAlpha(60));
   }

   Filter(UpgradeType upgrade, ChatFormatting color) {
      this(upgrade.getName(), territory -> territory.hasUpgrade(upgrade), color);
   }
   ...
   public String toText(Set<Filter> selected, Filter hovered) {
      String base = getName();
